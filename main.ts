...
const formattedDate:string = date.toLocaleDateString('en-GB');   // toLocaleDateString returns a formatted date
...
const person:any = {};    // I define the person as any to avoid errors. I should be defining an interface but will make do with this this time around. 
...
for (let i = 0; i < words.length; i++) {     //this is a for loop.  for (initializaionl; condition; increment)
  ...
  list1?.appendChild(li);     //there's that question mark again to avoid the null error. It seems to be okay to have the ?
}
